...
price <- range(diamonds$price)
carat <- range(diamonds$carat)
...
my.ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput('price.slider', label = "Price (in dollars)", min = min(price),
                  max = max(price), value = min(price)),
      ...
      sliderInput('carat.slider', label = "Carats", min = min(carat), 
                  max = max(carat), value = min(price)),

      # A checkboxInput labeled "Show Trendline". It's default value should be TRUE
      checkboxInput('checkbox.trend', label = "Show Trendline", value=TRUE),

      # A slectInput labeled "Facet By", with choices "cut", "clarity" and "color"
      selectInput('input.facet', label = "Facet By", choices = c("cut", "clarity", "color")),
...
# Define a Server function for the app
  ...
  # Assign a reactive `renderPlot()` function to the outputted `plot`
    ...
    # Hint: use dplyr and multiple `filter()` operations
      diamonds.filtered <- diamonds.sample %>% 
        filter(price == input$price.slider & carat == input$carat.slider)
    ...
    #   (hint: you can just pass that string to the `facet_wrap()` function!)
      ggplot(data = diamonds.filtered) +
        geom_point(mapping = aes(x = carat, y = price, color = clarity)) + 
        facet_wrap(by = input$input.facet)
